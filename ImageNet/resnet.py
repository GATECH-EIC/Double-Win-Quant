...
__all__ = ['ResNet', 'resnet18', 'resnet34', 'resnet50', 'resnet101',
           ...
           'wide_resnet50_2', 'wide_resnet101_2', 'ResNet50']
...
def conv3x3(in_planes: int, out_planes: int, stride: int = 1, groups: int = 1, dilation: int = 1) -> QConv2d:
    """3x3 convolution with padding"""
    return QConv2d(in_planes, out_planes, kernel_size=3, stride=stride,
                     padding=dilation, groups=groups, bias=False, dilation=dilation)


def conv1x1(in_planes: int, out_planes: int, stride: int = 1) -> QConv2d:
    """1x1 convolution"""
...
def _resnet(
    ...
    **kwargs: Any
    ...
    if pretrained:
        ...
        #     if 'bn' in name or 'downsample' in:
        #         print(name)
        # input()

        if kwargs['num_bits_list'] is not None:
            for key in state_dict.copy():
                if 'bn' in key or 'downsample.1' in key:
                    new_key_list = []

                    for i in range(len(kwargs['num_bits_list'])):
                        new_key = []
...
def ResNet50(num_bits_list=None, pretrained=False):
    global BatchNorm2d
    if num_bits_list is not None:
        BatchNorm2d = lambda num_features: USBatchNorm2d(num_features, num_bits_list)

    return _resnet('resnet50', Bottleneck, [3, 4, 6, 3], pretrained, progress=False,
