...
class UniformQuantize(Function):
    ...
    def forward(ctx, input, num_bits=None, qparams=None, flatten_dims=_DEFAULT_FLATTEN,
        ...
        with torch.no_grad():
            ...
            if stochastic:
                if type(stochastic) == bool:
                    noise = output.new(output.shape).uniform_(-0.5, 0.5)
                else:
                    noise = output.new(output.shape).uniform_(-stochastic, stochastic)
                output.add_(noise)
            # quantize
...
class QuantMeasure(nn.Module):
    ...
    def forward(self, input, num_bits, qparams=None, stochastic=False):
        ...
        if self.measure:
            ...
            q_input = quantize(input, qparams=qparams, dequantize=self.dequantize,
                               stochastic=stochastic, inplace=self.inplace)
            return q_input
...
class QConv2d(nn.Conv2d):
    ...
    def __init__(self, in_channels, out_channels, kernel_size,
                 stride=1, padding=0, dilation=1, groups=1, bias=False, momentum=0.1, num_bits=None, num_grad_bits=None):
        super(QConv2d, self).__init__(in_channels, out_channels, kernel_size,
                                      stride, padding, dilation, groups, bias)

        self.quantize_input = QuantMeasure(shape_measure=(1, 1, 1, 1), flatten_dims=(1, -1), momentum=momentum)
    ...
    def forward(self, input, num_bits=0, num_grad_bits=0, full_prec=False):
        # if num_bits == 0:
        ...
        if full_prec:
            ...
            if self.num_grad_bits is not None:
        ...
        # print(num_bits, num_grad_bits)

        weight_qparams = calculate_qparams(self.weight, num_bits=num_bits, flatten_dims=(1, -1),
                                           reduce_dim=None)
        qweight = quantize(self.weight, qparams=weight_qparams)

        qinput = self.quantize_input(input, num_bits, stochastic=self.stochastic)
...
class QLinear(nn.Linear):
    ...
    def __init__(self, in_features, out_features, bias=True, biprecision=False, num_bits=None, num_grad_bits=None):
        super(QLinear, self).__init__(in_features, out_features, bias)
        self.biprecision = biprecision
        self.quantize_input = QuantMeasure(shape_measure=(1,), flatten_dims=(1, -1), momentum=momentum)
    ...
    def forward(self, input, num_bits=0, num_grad_bits=0):
        if self.num_bits is not None:
            num_bits = self.num_bits
        ...
        if not self.biprecision or num_bits_grad is None:
            output = F.linear(qinput, qweight, qbias)
            if num_bits_grad is not None:
                output = quantize_grad(
                    output, num_bits=num_bits_grad)
        else:
            output = linear_biprec(qinput, qweight, qbias, num_bits_grad)
        return output
